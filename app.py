...
st.markdown("Simulated Telemetry Proof of Concept: Historical OEE and degradation analytics across 50 assets.")
...
col2.metric("Availability", f"{latest_data['Availability'] * 100:.1f}%")
col3.metric("Performance", f"{latest_data['Performance'] * 100:.1f}%")
col4.metric("Quality", f"{latest_data['Quality'] * 100:.1f}%")

st.caption(f"⚠️ **CRITICAL FLEET AVERAGES:** {latest_data.get('FAILURE', 0):.1f} minutes of downtime | {latest_data.get('defective_parts', 0):.1f} defective parts produced.")
